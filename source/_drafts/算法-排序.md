...
# 选择排序
思想：从数组中选择最小元素，并将其与第一个元素交换位置。再从数组中剩下的元素中选择出最小元素，将其与数组的第二个元素交换位置。不断进行这样的操作，直到将整个数组排序。
...
# 插入排序
## 算法思想
每次都将当前元素插入到左侧已经排序的数组中，使得插入之后左侧数组依然有序。（如果待插入的元素与左侧有序序列中的某个元素相等，则将待插入元素插入到相等元素的后面。）
